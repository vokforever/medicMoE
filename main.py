...
class TestAnalysisAgent:
    ...
    async def get_test_summary(self, user_id: int, test_names: List[str] = None) -> str:
        ...
        try:
            ...
            cached = supabase.table("doc_agent_cache").select("*").eq("user_id", user_id).eq("query", cache_key).execute()
...
async def generate_answer(question: str, context: str = "", history: List[Dict[str, str]] = None, patient_data: Dict[str, Any] = None, user_id: int = None) -> str:
    models_to_try = [
...
async def history_command(message: types.Message):
    try:
        response = supabase.table("doc_user_feedback").select("*").eq("user_id", message.from_user.id).order("created_at", desc=True).limit(5).execute()
...
async def handle_document(message: types.Message, state: FSMContext):
    ...
    if message.document.mime_type == "application/pdf":
        ...
        if pdf_text:
            ...
            await processing_msg.edit_text("😔 Не удалось извлечь текст из PDF файла. Пожалуйста, попробуйте другой файл.")
    else:
...
@dp.callback_query(F.data.in_(["clarify_question", "upload_tests", "try_again", "analyze_pdf", "create_extracted_profile", "manual_profile"]))
    ...
    if callback.data == "clarify_question":
        ...
        if patient_data and (patient_data.get("name") or patient_data.get("age") or patient_data.get("gender")):
            ...
            if missing_data:
                ...
                if create_patient_profile(callback.from_user.id, patient_data['name'], patient_data['age'], patient_data['gender']):
                    await callback.message.edit_text(
                        f"✅ Профиль успешно создан!\n\n"
                        f"👤 <b>Ваш профиль:</b>\n"
                    ...
                    await callback.message.edit_text("😔 Не удалось создать профиль. Пожалуйста, попробуйте ввести данные вручную с помощью команды /profile.")
        else:
            await callback.message.edit_text("😔 Не удалось извлечь данные пациента. Пожалуйста, создайте профиль вручную с помощью команды /profile.")
